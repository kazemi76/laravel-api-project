...
class PaymentController extends ApiController
{
    ...
    public function send(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'order_items.*.product_id' => 'required|integer', //order item is array .check item of array by .*.
        ...
        foreach ($request->order_items as $orderItem) {
            ...
            if ($product->quantity < $orderItem['quantity']) {
                return $this->errorResponse('تعداد بالاتر از حد مجاز است', 422);
            }
        ...
        if ($result->status) {
            ...
            OrderController::create($request,$mounts,$result->token);
            $go = "https://pay.ir/pg/$result->token";
            return $this->successResponse(['url' => $go]);
        } else {
        ...
        if (isset($result->status)) {
            if ($result->status == 1) {
                if(Transaction::where('trans_id',$result->transId)->exists()){
                    return $this->errorResponse('این تراکنش قبلا در سیستم ثبت شده!', 422);
                }
